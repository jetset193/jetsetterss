-- Flight Subscription System Tables (Simplified Version)
-- For Supabase database without auth.users dependency
...
-- Create simple RLS policies without admin checks
...
-- Simple policies for public access where needed
CREATE POLICY "Public read access to flight deals" ON flight_deals FOR SELECT TO anon USING (true);
CREATE POLICY "Public read access to promotional offers" ON promotional_offers FOR SELECT TO anon USING (true);
CREATE POLICY "Public read access to subscriber avatars" ON subscriber_avatars FOR SELECT TO anon USING (true);

-- Policy for public subscription access
CREATE POLICY "Anyone can subscribe" ON email_subscribers FOR INSERT TO anon WITH CHECK (true);
